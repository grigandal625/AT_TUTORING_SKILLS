...
from at_tutoring_skills.apps.skills.dto import Skill
from at_tutoring_skills.apps.skills.dto import SkillRelation
from at_tutoring_skills.apps.skills.dto import UserSkill


def build_skill_graph(skills: list[Skill], relations: list[SkillRelation], user_skills: list[UserSkill]) -> Digraph:
...
def build_legend_graph(skills: list[Skill], relations: list[SkillRelation], user_skills: list[UserSkill]) -> Digraph:
    ...
    dot.node("legend", label=legend_content, shape="none", fontsize="10", fontname="Arial")

    return dot


def _generate_legend_content(
    skills: list[Skill], user_marks: dict[int, float], relation_types: dict, root_nodes: list[str]
) -> str:
...
if __name__ == "__main__":
    ...
    relations = [
        ...
        SkillRelation(
            pk=6, source_skill_id=2, target_skill_id=7, relation_type=1
        ...
        SkillRelation(
            pk=15, source_skill_id=18, target_skill_id=19, relation_type=2
        ),  # Коммуникация → Работа в команде
        SkillRelation(
            pk=16, source_skill_id=18, target_skill_id=20, relation_type=2
        ),  # Коммуникация → Управление временем
        ...
        SkillRelation(
            pk=22, source_skill_id=20, target_skill_id=1, relation_type=4
        ),  # Управление временем ⇢ Программирование
