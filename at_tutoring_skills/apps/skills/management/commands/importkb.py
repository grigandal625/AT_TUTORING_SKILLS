...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        data_dir = Path("C:/Users/Milana/at_marks/AT_TUTORING_SKILLS/at_tutoring_skills/apps/skills/management/commands/data")
            ...
            for tasks_file in tasks_files:
                ...
                if not variant_name:
                    self.stdout.write(self.style.ERROR(
                        f"Файл {tasks_file.name} не содержит variant_name, пропускаем"
                    ))
                    continue
                    ...
                    for code in task_data.get('skill_codes', []):
                        if code in skills_map:
                            ...
                            self.stdout.write(self.style.WARNING(
                                f"  Навык с кодом {code} не найден для задания {task.task_name}"
                            ))
