...
class SkillService:
    ...
    def _calc_skill(self, user, skill):
        ...
        mistakes = list(Mistake.objects.filter(user=user, skills__in=[skill]).exclude(fine__isnull=True))
    ...
    def _can_calculate_skill(self, user, skill):
        source_skills = SKillConnection.objects.filter(skill_to=skill).values_list("skill_from", flat=True)
        existing_skills_count = UserSkill.objects.filter(user=user, skill__in=source_skills).count()
    ...
    def _calculate_weighted_mark(self, user, skill):
        ...
        result = (
            UserSkill.objects.filter(user=user, skill__in=connections.values("skill_from"))
            .annotate(weighted=F("mark") * F("skill__outputs__weight"))
            .aggregate(total_weighted=Sum("weighted"), total_weights=Sum("skill__outputs__weight"))
        )
    ...
    def _update_user_skill(self, user, skill, mark):
        user_skill, _ = UserSkill.objects.update_or_create(user=user, skill=skill, defaults={"mark": mark})
        return user_skill
    ...
    async def iterative_update_user_skills(self, user: User, max_iterations: int = 100) -> Dict:
        ...
        while iterations < max_iterations:
            ...
            if updated_in_iteration == 0:
                return {"iterations": iterations, "updated": total_updated, "converged": True}

        return {"iterations": iterations, "updated": total_updated, "converged": False}

    # Методы для обработки и вывода результатов

    async def process_and_get_skills(
        self, user: User, task_object: Union[int, SUBJECT_CHOICES, List[Union[int, SUBJECT_CHOICES]]] = None
    ) -> str:
