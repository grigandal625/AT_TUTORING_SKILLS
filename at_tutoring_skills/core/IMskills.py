...
class ATTutoringIMSkills(ATComponent):
    ...
    def im_resources_from_dict(self, resource: dict, event: str, auth_token) -> dict:
        ...
        if resource:
            ...
            new_resource["resourceTypeName"] = self.get_smth_by_key(
                "im_resourceTypes", "typeId", new_resource["resourceTypeId"], auth_token
            )
    ...
    def im_func_from_dict(func: dict, event: str) -> dict:
        ...
        if func:
            ...
            for param in func.get("params", []):
                processed_param = {"paramName": param["name"], "paramType": param["type"]}
        ...
        if dict1.get("rel_resources", None) is not None:
            for res in dict1["rel_resources"]:
                rel_res = {
                    ...
                    "resource_type_name": self.get_smth_by_key(
                        "im_resourceTypes", "typeId", res.get("resource_type_id"), auth_token
                    ),
                }
    ...
    async def handle_im_resourceTypes_updated(self, event: str, data: dict, auth_token: str):
        ...
        return {
            "stage_done": True,
            "message": "Этап создания типов ресурсов пройден. Осуществляется переход к этапу создания ресурсов.",
        }
