...
# # from delivery.editor.resource.models.models import *
...
#     cash: dict = None
#     repository = None
#     resource_types_service = None
#     resources_service = None
#     templates_service = None
#     template_usage_service = None
#     funcs_service = None

#     def __init__(self, connection_parameters: ConnectionParameters, *args, **kwargs):
#         super().__init__(connection_parameters, *args, **kwargs)
#         self.repository = Repository
#         self.skills = {} # временное хранилище
#         self.cash = {}       
#         self.resource_types_service = IMResourceTypesService(self.repository)
#         self.resources_service = IMResourcesService(self.repository)
#         self.template_usage_service = IMTemplatesService(self.repository)
#         self.funcs_service = IMTemplateUsagesService(self.repository)
#         self.templates_service = IMFuncsService(self.repository)
...
# #         self.add_to_cash('im_resourceTypes', 'resourceTypeId', data, auth_token)

# #     def remove_im_resourceType_from_cash(self, key_val: str, auth_token: str):
# #         self.remove_from_cash(key_val, 'im_resourceTypes', 'resourceTypeId', auth_token)

# #     # Ресурс
# #     def add_im_resource_to_cash(self, data: dict, auth_token: str):
# #         self.add_to_cash('im_resources', 'resourceId', data, auth_token)

# #     def remove_im_resource_from_cash(self, key_val: str, auth_token: str):
# #         self.remove_from_cash(key_val, 'im_resources', 'resourceId', auth_token)

# #     # Образец операции
# #     def add_im_template_to_cash(self, data: dict, auth_token: str):
# #         self.add_to_cash('im_templates', 'templateId', data, auth_token)

# #     def remove_im_template_from_cash(self, key_val: str, auth_token: str):
# #         self.remove_from_cash(key_val, 'im_templates', 'templateId', auth_token)

# #     # Операция
# #     def add_im_templateUsage_to_cash(self, data: dict, auth_token: str):
# #         self.add_to_cash('im_templateUsages', 'templateUsageId', data, auth_token)

# #     def remove_im_templateUsage_from_cash(self, key_val: str, auth_token: str):
# #         self.remove_from_cash(key_val, 'im_templateUsages', 'templateUsageId', auth_token)

# #     # Функция
# #     def add_im_func_to_cash(self, data: dict, auth_token: str):
# #         self.add_to_cash('im_funcs', 'funcId', data, auth_token)

# #     def remove_im_func_from_cash(self, key_val: str, auth_token: str):
# #         self.remove_from_cash(key_val, 'im_funcs', 'funcId', auth_token)
...
# # # Инициализация вводимых данных
# # # 1) =================================== im ======================================== 
# #     @authorized_method
# #     async def handle_im_created(self, event: str, data: dict, auth_token: str):
# #         print('Обучаемый создал пустую модель (ИМ)')
# #         user_id = self.get_user_id_or_token(self, auth_token)
# #         self.init_cash(user_id)

# #     @authorized_method
# #     async def handle_im_updated(self, event: str, data: dict, auth_token: str):
# #         print('Обучаемый обновил модель (ИМ)')
# #         user_id = self.get_user_id_or_token(self, auth_token)
# #         self.init_cash(user_id)
...
# # 2) ============================= Resource Types ===================
# @authorized_method
# async def handle_im_resourceTypes_created(self, event: str, data: dict, auth_token: str) -> None:
#     print('Обучаемый создал тип ресурса (ИМ): ', data)
#     pass

# @authorized_method
# async def handle_im_resourceTypes_updated(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         kb_type = IMTypeService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Type Updated: Syntax Mistakes: {e}") from e

#     try:
#         IMTypeService.handle_logic_mistakes(user_id, kb_type)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Type Updated: Logic Mistakes: {e}") from e

#     try:
#         TaskService.complete_task(user_id, event, kb_type.id)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Type Updated: Complete Task: {e}") from e

# # 3) ============================= Resources ===================
# @authorized_method
# async def handle_im_resources_created(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         resource = IMResourceService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Resource Created: Syntax Mistakes: {e}") from e

#     try:
#         IMResourceService.handle_logic_mistakes(user_id, resource)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Resource Created: Logic Mistakes: {e}") from e

#     try:
#         TaskService.complete_task(user_id, event, resource.id)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Resource Created: Complete Task: {e}") from e


# @authorized_method
# async def handle_im_resources_updated(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         resource = IMResourceService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Resource Updated: Syntax Mistakes: {e}") from e

#     try:
#         IMResourceService.handle_logic_mistakes(user_id, resource)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Resource Updated: Logic Mistakes: {e}") from e

#     try:
#         TaskService.complete_task(user_id, event, resource.id)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Resource Updated: Complete Task: {e}") from e


# # 4) ============================= Templates ===================
# @authorized_method
# async def handle_im_template_created(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         template = IMTemplateService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Created: Syntax Mistakes: {e}") from e

#     try:
#         IMTemplateService.handle_logic_mistakes(user_id, template)
#     except BaseException as e:
#                 raise ValueError(f"Handle IM Template Created: Logic Mistakes: {e}") from e

#     try:
#         TaskService.complete_task(user_id, event, template.id)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Created: Complete Task: {e}") from e


# @authorized_method
# async def handle_im_template_updated(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         template = IMTemplateService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Updated: Syntax Mistakes: {e}") from e

#     try:
#         IMTemplateService.handle_logic_mistakes(user_id, template)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Updated: Logic Mistakes: {e}") from e

#     try:
#         TaskService.complete_task(user_id, event, template.id)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Updated: Complete Task: {e}") from e


# # 5) ============================= Template Usages ===================
# @authorized_method
# async def handle_im_templateUsages_created(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         template_usage = IMTemplateUsageService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
...
# @authorized_method
# async def handle_im_templateUsages_updated(self, event: str, data: dict, auth_token: str) -> None:
#     user_id = self.AT_USER.parse_auth_token(auth_token)

#     try:
#         template_usage = IMTemplateUsageService.handle_syntax_mistakes(user_id, data)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Usage Updated: Syntax Mistakes: {e}") from e

#     try:
#         IMTemplateUsageService.handle_logic_mistakes(user_id, template_usage)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Usage Updated: Logic Mistakes: {e}") from e

#     try:
#         TaskService.complete_task(user_id, event, template_usage.id)
#     except BaseException as e:
#         raise ValueError(f"Handle IM Template Usage Updated: Complete Task: {e}") from e
...
# #             kb_type = IMTypeService.handle_syntax_mistakes(user_id, data)
# #         except BaseException as e:
# #             raise ValueError(f"Handle IM Type Created: Syntax Mistakes: {e}") from e

# #         try:
# #             IMTypeService.handle_logic_mistakes(user_id, kb_type)
# #         except BaseException as e:
# #             raise ValueError(f"Handle IM Type Created: Logic Mistakes: {e}") from e
...
# #             TaskService.complete_task(user_id, event, kb_type.id)
# #         except BaseException as e:
# #             raise ValueError(f"Handle IM Type Created: Complete Task: {e}") from e
...
# #             resource_type_request = ResourceTypeRequest(**data)

# #         except ValidationError as e:
# #             print(f"Ошибка валидации Pydantic структуры: {e.json()}")
# #             return {'stage_done': False, 'message': f"Ошибка в структуре данных: {e.json()}", 'errors': [e.json()]} # Возвращаем детали валидации Pydantic
...
# #         # Получаем эталонный ресурс из БД по виду сущности и имени
# #         etalon_type = self.get_etalon_resource_type(event, resourcetype_dict.name)
        
        
# #             # print('Проверка resourcetype_dict:', resourcetype_dict) # Вывод для проверки
# #             # self.check_skills(event, data, auth_token)
# #             # self.add_im_resourceTypes_to_cash(resourcetype_dict, auth_token) # запись в кэш
