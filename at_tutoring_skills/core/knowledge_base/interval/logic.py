...
from at_krl.core.temporal.allen_operation import AllenOperation, TEMPORAL_TAGS_SIGNS, get_inversion
if TYPE_CHECKING:
...
class KBIntervalServiceLogicLexic:  # Изменено с KBEventServiceLogicLexic
    ...
    def get_various_references(
        ...
        if isinstance(condition, SimpleOperation):
            ...
            if condition.sign in {'eq', 'ne','add', 'mul','and', 'or', 'xor' }:
                variations.append(SimpleOperation(sign=condition.sign, 
            ...
            if condition.sign in {'add', 'mul','and', 'or', 'xor'}:
                if isinstance(condition.left, SimpleOperation):
                    if condition.left.sign == condition.sign:
                        op = SimpleOperation(sign=condition.left.sign, 
                                            left=condition.left.left, 
                                            right=condition.right)
                        variations.append(SimpleOperation(sign=condition.sign, 
                                                  right=condition.left.right, 
                                                  left=op))
                ...
                if isinstance(condition.right, SimpleOperation):
                    if condition.right.sign == condition.sign:
                        op = SimpleOperation(sign=condition.right.sign, 
                                            left=condition.right.right, 
                                            right=condition.left)
                        variations.append(SimpleOperation(sign=condition.sign, 
                                                  left=condition.right.right, 
                                                  right=op))
    ...
    def estimate_interval(self, user_id: str, task_id: int, etalon_interval: KBInterval, interval: KBInterval, context: Context):  # Изменено estimate_event на estimate_interval
        print("Estimate interval")  # Изменено сообщение
        errors_list = []

        check_open = self.estimate_open_condition(
            user_id, task_id, etalon_interval.open, interval.open, context
        )
        check_close = self.estimate_close_condition(
            user_id, task_id, etalon_interval.open, interval.open, context
        )

    async def handle_logic_lexic_mistakes(
        self: "KBIntervalService", user: User, task: Task, interval: KBInterval, interval_et: KBInterval  # Изменено параметры
