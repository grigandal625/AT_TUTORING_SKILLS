...
# ==============================get from dict=================== проверено 
    ...
    def kb_object_from_dict(self, object_dict: dict, event: str, auth_token: str)-> dict:
        ...
        if object_dict:
            ...
            if event != "kbObjects/create" and event != "kbObjects/delete":
                new_object_dict['properties'] = [
                    {
                        'id' : item['kb_id'],
                        'type' : self.get_type_name_by_id(item['type'], auth_token), # впихнуть функцию имя по id
...
# ============================= type ===================
    ...
    async def handle_kb_type_updated(self, event: str, data: dict, auth_token: str):
        ...
        return {'skills': current_skills, 'stage_done': False, 'message': 'Выполнены не все задания'}
