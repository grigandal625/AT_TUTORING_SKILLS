...
from .models import Skill, Task, Reaction, ScenarioPart, Event  # Импорт всех моделей
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for scenario_part_data in scenario_parts_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Часть сценария для "{scenario_part.scenario_name}" успешно создана.'))
            else:
                self.stdout.write(self.style.SUCCESS(f'Часть сценария для "{scenario_part.scenario_name}" уже существует.'))
